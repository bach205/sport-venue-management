...
export function PostCard({ post, onContactNow }: PostCardProps) {
  ...
  const totalPlayers = post.playersNeeded || 1; // numberOfPlayers = tổng số người muốn có
  const currentPlayers = post.currentPlayers;
  const spotsLeft = Math.max(0, totalPlayers - currentPlayers);
  const matched = !!post.isMatch;

  return (
    <div
      className="bg-white rounded-2xl flex flex-col overflow-hidden transition-shadow hover:shadow-lg"
      style={{
        border: matched ? "1.5px solid #006a65" : "1px solid #dfc0b3",
        boxShadow: matched
          ? "0 2px 16px rgba(0,106,101,0.12)"
          : "0 2px 12px rgba(36,25,20,0.07)",
      }}
    >
      {/* Card header: sport color band — green when matched */}
      <div
        className="h-1.5 w-full"
        style={{
          background: matched
            ? "linear-gradient(90deg, #006a65 0%, #34b27b 100%)"
            : "linear-gradient(90deg, #a04100 0%, #ff7e36 100%)",
        }}
      />
      ...
      <div className="px-5 pb-5">
        {matched ? (
          <button
            className="w-full h-11 rounded-xl flex items-center justify-center gap-2 cursor-default"
            style={{
              background: "linear-gradient(90deg, #e6f9f5 0%, #d4f4ed 100%)",
              fontFamily: "Lexend, sans-serif",
              fontSize: "14px",
              fontWeight: 700,
              color: "#006a65",
